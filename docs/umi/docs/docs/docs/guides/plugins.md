...
Question 3: 在 plugin 中可以返回 plugins 或 presets 吗？

Answer 3: 是的，在 plugin 中可以返回 plugins 或 presets。但是，Umi 不会对返回的 plugins 或 presets 做任何处理。返回的 plugins 或 presets 只是为了提供给其他插件或 preset 使用。
